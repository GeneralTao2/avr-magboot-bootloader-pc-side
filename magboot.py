...
# Edited by Diulgher Artiom (artiom.diulgher@gmail.com)
...
def main(argv):
    ...
    try:
        dev = devicelist[argv[1]]
    except KeyError:
        print("Unsupported device '" + argv[1] + "'")
        sys.exit(3)

    ser = serial.Serial(port=argv[0], baudrate=250000, timeout=1)
    time.sleep(0.1) 

    try:
        opts, args = getopt.getopt(argv[2:], 'a:w:ijrzd')
